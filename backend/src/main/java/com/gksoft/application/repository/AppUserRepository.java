...
public interface AppUserRepository extends JpaRepository<AppUser, Long> {
    ...
    @Query(
        value = "select distinct appUser from AppUser appUser left join fetch appUser.country left join fetch appUser.stateProvince left join fetch appUser.country",
    ...
    @Query(
        "select distinct appUser from AppUser appUser left join fetch appUser.country left join fetch appUser.stateProvince left join fetch appUser.country"
    )
    List<AppUser> findAllWithToOneRelationships();

    @Query(
        "select appUser from AppUser appUser left join fetch appUser.country left join fetch appUser.stateProvince left join fetch appUser.country where appUser.id =:id"
    )
